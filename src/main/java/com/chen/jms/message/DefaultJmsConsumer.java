...
import com.chen.jms.configuration.AMQBeanFactoryHolder;
...
public class DefaultJmsConsumer implements JmsConsumerMapping {
    ...
    public List<String> getCustomAnnotation() {
        return getBeansWithAnnotation();
    }

    private List<String> getBeansWithAnnotation() {
        List<String> beans = new ArrayList<>();
        ConfigurableListableBeanFactory factory = (ConfigurableListableBeanFactory) AMQBeanFactoryHolder.get();
        for(String name : factory.getBeanDefinitionNames()) {
            BeanDefinition bd = factory.getBeanDefinition(name);
            if( bd instanceof ScannedGenericBeanDefinition) {
                boolean has = ((ScannedGenericBeanDefinition)bd).getMetadata().hasAnnotation(CustomJmsListener.class.getName());
                if (has) {
                    beans.add(name);
                }
            }
    ...
    private void init() {
        ...
        for (String name : beans) {
            Object bean = AMQApplicationHolder.getBean(name);
            ...
            for (Method method : methods) {
                ...
                for (int i = 0; i <annotations.length; i++) {
                    for (Annotation a : annotations[i]) {
                        if (a.annotationType().isAssignableFrom(CustomMapping.class)) {
                            annotationIndex = i;
                        }
                    }
                }
